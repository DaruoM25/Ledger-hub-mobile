...
### Points d'attention transmis
- **Un seul enregistrement malformé rend toute la liste illisible.** Constaté en recette en injectant des factures sans ligne : `Invoice.init` exige au moins une ligne, `runCatching` transforme l'échec en `Result.failure`, et `seedDemoDataIfEmpty` — qui lit avec `getOrDefault(emptyList())` — conclut que la base est vide et **réécrit les sept factures de démonstration par `INSERT OR REPLACE` à chaque lancement**, détruisant les statuts réels. Le semis n'existe qu'en démonstration, mais le mécanisme sous-jacent est le même qu'en A-01 : une erreur avalée qui se transforme en perte de données silencieuse. **À traiter en priorité** : soit ignorer la ligne fautive plutôt que l'ensemble, soit distinguer « base vide » de « lecture en échec ».
- **`PRAGMA foreign_keys` n'est toujours pas activé.** La suppression directe d'une facture laisse ses entrées d'audit orphelines, malgré le `ON DELETE CASCADE` déclaré. Observé en recette sur les lignes de test supprimées en SQL brut.
